...
from extraction import inception_v4
from extraction import inception_v3
from extraction import vgg
...
if __name__ == '__main__':
    parser = argparse.ArgumentParser(description='Vectorizing images through taking' \
                                                  ' a layer from pretrained model.')
...
def main():
    ...
    images = (load_image(image_path, ARGS) for image_path in image_paths)
    ...
    with tf.Session() as sess:
        ...
        length = np.floor(len(image_paths) / BATCH_SIZE) + \
                 int(bool(len(image_paths) / BATCH_SIZE))
        idx = 0
